...
**Symptoms:** "permission denied" errors when reading/writing mounted shares

**Solutions:**
1. The container runs as root, so most permission issues stem from host-side mount configuration
2. Confirm the mount path exists and is accessible:
   ```bash
   ls -l /path/to/share
   ```
3. Adjust host ownership if needed:
   ```bash
   sudo chown -R <uid>:<gid> /path/to/share
   ```
4. Review detailed guidance: [docs/CONFIGURATION.md](docs/CONFIGURATION.md)
